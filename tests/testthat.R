...
test_that("fpsc_calculate_111", {
  data[1:length(data)] <- 0
  data[as.character(seq(chron("01/02/90"), chron("01/12/90")))] <- 2 #31+28+1
  data[as.character(seq(chron("03/01/90"), chron("03/31/90")))] <- 3
  data[as.character(seq(chron("01/02/00"), chron("01/12/00")))] <- 2 #31+29+1
  data[as.character(seq(chron("03/01/00"), chron("03/31/00")))] <- 3
  data[as.character(seq(chron("05/20/01"), chron("05/24/01")))] <- 20 #31+28+31+30+20
  expect_equivalent(as.numeric(calculate_111(data)["2000"]), 31+29+1)
})

test_that("lpsc_calculate_112", {
  data[1:length(data)] <- 0
  data[as.character(seq(chron("01/02/90"), chron("01/12/90")))] <- 2 #31+28+31
  data[as.character(seq(chron("03/01/90"), chron("03/31/90")))] <- 3
