...
index_MA <- function(Temperature, Rain, DewPoint, MAP, Wind, KBDI){
	McA<-0.0
	McA_list <- c()
	...
	for(day in 1:llength) {
		if(!is.na(sum(Temperature[day], Rain[day], DewPoint[day], Wind[day], KBDI[day]))){
			if(Rain[day] > 0.0){
				LastRain <- Rain[day]
				DSinceRain <- 0
			}else{
...
index_KBDI <- function(Temperature, Rain, MAP){
	...
	for(day in 1:llength){
		# print(day)
		if(!is.na(Temperature[day] + Rain[day])){
			if(is.na(KBDI)){
				KBDI = 400.0
			}
			if(rainYD == 0.0){
...
index_MMI <- function(DewPoint,Temperature,Rain){
	...
	for(day in 1:llength){
		# print(day)
		if(!is.na(Rain[day] + DewPoint[day] + Temperature[day])){
			if (Rain[day] > 3.0){
				M <- 0.0
...
index_CFWI <- function(Month,Days,Temp,Dew,WS,Rain,daylist){
	...
	llength <- length(Temp)
	for(day in 1:llength){
		if(!is.na(Temp[day] + Dew[day] + WS[day] + Rain[day])){
			T = Temp[day]
			H = Humid[day]
			if(H > 100){H=100}
			W = WS[day]
